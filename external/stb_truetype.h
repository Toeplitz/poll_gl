 ...
 int main(int argc, char **argv) 
{
  ...
  int w, h, i, j, c = (argc > 1 ? atoi(argv[1]) : 'a'), s =
  ...
  bitmap =
    stbtt_GetCodepointBitmap(&font, 0, stbtt_ScaleForPixelHeight(&font, s), c,
                             &w, &h, 0, 0);
   for (j = 0; j < h; ++j) {
    for (i = 0; i < w; ++i)
...
// the stack or as a global or etc, but you should treat it as opaque.
    ...
    extern int stbtt_InitFont(stbtt_fontinfo * info, const unsigned char *data,
                              int offset);
...
//
  extern float stbtt_ScaleForPixelHeight(const stbtt_fontinfo * info,
                                          float pixels);
...
// so if you prefer to measure height by the ascent only, use a similar calculation.
  extern float stbtt_ScaleForMappingEmToPixels(const stbtt_fontinfo * info,
                                                float pixels);
  
// computes a scale factor to produce a font whose EM size is mapped to
// 'pixels' tall. This is probably what traditional APIs compute, but
// I'm not positive.
  extern void stbtt_GetFontVMetrics(const stbtt_fontinfo * info, int *ascent,
...
//   the scale factor for a given size
  extern void stbtt_GetFontBoundingBox(const stbtt_fontinfo * info, int *x0,
                                        int *y0, int *x1, int *y1);
  
// the bounding box around all possible characters
  extern void stbtt_GetCodepointHMetrics(const stbtt_fontinfo * info,
...
//   these are expressed in unscaled coordinates
  extern int stbtt_GetCodepointKernAdvance(const stbtt_fontinfo * info,
                                            int ch1, int ch2);
  
// an additional amount to add to the 'advance' value between ch1 and ch2
// @TODO; for now always returns 0!
  extern int stbtt_GetCodepointBox(const stbtt_fontinfo * info, int codepoint,
                                    int *x0, int *y0, int *x1, int *y1);
  
// Gets the bounding box of the visible part of the glyph, in unscaled coordinates
  extern void stbtt_GetGlyphHMetrics(const stbtt_fontinfo * info,
                                      int glyph_index, int *advanceWidth,
                                      int *leftSideBearing);
  extern int stbtt_GetGlyphKernAdvance(const stbtt_fontinfo * info, int glyph1,
                                        int glyph2);
  extern int stbtt_GetGlyphBox(const stbtt_fontinfo * info, int glyph_index,
                                int *x0, int *y0, int *x1, int *y1);
...
// returns non-zero if nothing is drawn for this glyph
  extern int stbtt_GetCodepointShape(const stbtt_fontinfo * info,
...
//   these are expressed in "unscaled" coordinates
  extern void stbtt_FreeShape(const stbtt_fontinfo * info,
                               stbtt_vertex * vertices);
...
// frees the bitmap allocated below
  extern unsigned char *stbtt_GetCodepointBitmap(const stbtt_fontinfo * info,
...
// xoff/yoff are the offset it pixel space from the glyph origin to the top-left of the bitmap
  extern unsigned char *stbtt_GetCodepointBitmapSubpixel(const stbtt_fontinfo *
                                                          info, float scale_x,
                                                          float scale_y,
                                                          float shift_x,
                                                          float shift_y,
...
// shift for the character
  extern void stbtt_MakeCodepointBitmap(const stbtt_fontinfo * info,
...
// shift for the character
  extern void stbtt_GetCodepointBitmapBox(const stbtt_fontinfo * font,
...
// on glyph indices instead of Unicode codepoints (for efficiency)
  extern unsigned char *stbtt_GetGlyphBitmap(const stbtt_fontinfo * info,
                                             float scale_x, float scale_y,
                                             int glyph, int *width, int *height,
                                             int *xoff, int *yoff);
  extern unsigned char *stbtt_GetGlyphBitmapSubpixel(const stbtt_fontinfo *
                                                      info, float scale_x,
                                                      float scale_y,
                                                      float shift_x,
                                                      ...
                                                      int *xoff, int *yoff);
  extern void stbtt_MakeGlyphBitmap(const stbtt_fontinfo * info,
                                     unsigned char *output, int out_w,
  ...
  extern void stbtt_MakeGlyphBitmapSubpixel(const stbtt_fontinfo * info,
                                             ...
                                             int glyph);
  extern void stbtt_GetGlyphBitmapBox(const stbtt_fontinfo * font, int glyph,
                                       float scale_x, float scale_y, int *ix0,
                                       int *iy0, int *ix1, int *iy1);
  extern void stbtt_GetGlyphBitmapBoxSubpixel(const stbtt_fontinfo * font,
...
//             You have to have called stbtt_InitFont() first.
  extern int stbtt_FindMatchingFont(const unsigned char *fontdata,
...
// the second string interpreted as big-endian utf16... useful for strings from next func
  extern const char *stbtt_GetFontNameString(const stbtt_fontinfo * font,
...
//     http://www.microsoft.com/typography/otspec/name.htm
  enum {                       // platformID
    STBTT_PLATFORM_ID_UNICODE = 0, STBTT_PLATFORM_ID_MAC =
      1, STBTT_PLATFORM_ID_ISO = 2, STBTT_PLATFORM_ID_MICROSOFT = 3 
  };
  enum {                      // encodingID for STBTT_PLATFORM_ID_UNICODE
    STBTT_UNICODE_EID_UNICODE_1_0 = 0, STBTT_UNICODE_EID_UNICODE_1_1 =
      1, STBTT_UNICODE_EID_ISO_10646 = 2, STBTT_UNICODE_EID_UNICODE_2_0_BMP =
      3, STBTT_UNICODE_EID_UNICODE_2_0_FULL = 4 
  };
  enum {                      // encodingID for STBTT_PLATFORM_ID_MICROSOFT
    STBTT_MS_EID_SYMBOL = 0, STBTT_MS_EID_UNICODE_BMP =
      1, STBTT_MS_EID_SHIFTJIS = 2, STBTT_MS_EID_UNICODE_FULL = 10 
  };
  enum {                      // encodingID for STBTT_PLATFORM_ID_MAC; same as Script Manager codes
    STBTT_MAC_EID_ROMAN = 0, STBTT_MAC_EID_ARABIC = 4, STBTT_MAC_EID_JAPANESE =
      1, STBTT_MAC_EID_HEBREW = 5, STBTT_MAC_EID_CHINESE_TRAD =
      2, STBTT_MAC_EID_GREEK = 6, STBTT_MAC_EID_KOREAN =
      3, STBTT_MAC_EID_RUSSIAN = 7 
  };
  enum {                      // languageID for STBTT_PLATFORM_ID_MICROSOFT; same as LCID...
    // problematic because there are e.g. 16 english LCIDs and 16 arabic LCIDs
    STBTT_MS_LANG_ENGLISH = 0x0409, STBTT_MS_LANG_ITALIAN =
      0x0410, STBTT_MS_LANG_CHINESE = 0x0804, STBTT_MS_LANG_JAPANESE =
      0x0411, STBTT_MS_LANG_DUTCH = 0x0413, STBTT_MS_LANG_KOREAN =
      0x0412, STBTT_MS_LANG_FRENCH = 0x040c, STBTT_MS_LANG_RUSSIAN =
      0x0419, STBTT_MS_LANG_GERMAN = 0x0407, STBTT_MS_LANG_SPANISH =
      0x0409, STBTT_MS_LANG_HEBREW = 0x040d, STBTT_MS_LANG_SWEDISH = 0x041D 
  };
  enum {                      // languageID for STBTT_PLATFORM_ID_MAC
    STBTT_MAC_LANG_ENGLISH = 0, STBTT_MAC_LANG_JAPANESE =
      11, STBTT_MAC_LANG_ARABIC = 12, STBTT_MAC_LANG_KOREAN =
      23, STBTT_MAC_LANG_DUTCH = 4, STBTT_MAC_LANG_RUSSIAN =
      32, STBTT_MAC_LANG_FRENCH = 1, STBTT_MAC_LANG_SPANISH =
      6, STBTT_MAC_LANG_GERMAN = 2, STBTT_MAC_LANG_SWEDISH =
      5, STBTT_MAC_LANG_HEBREW = 10, STBTT_MAC_LANG_CHINESE_SIMPLIFIED =
      33, STBTT_MAC_LANG_ITALIAN = 3, STBTT_MAC_LANG_CHINESE_TRAD = 19 
  };
                                   ...
                                   int index) 
{
 ...
 int stbtt_InitFont(stbtt_fontinfo * info, const unsigned char *data2,
                      int fontstart) 
{
 ...
 int stbtt_FindGlyphIndex(const stbtt_fontinfo * info,
                            int unicode_codepoint) 
{
 ...
 int stbtt_GetCodepointShape(const stbtt_fontinfo * info,
                               int unicode_codepoint,
                               stbtt_vertex ** vertices) 
{
 ...
 static void stbtt_setvertex(stbtt_vertex * v, stbtt_uint8 type,
                               stbtt_int32 x, stbtt_int32 y, stbtt_int32 cx,
                               stbtt_int32 cy) 
{
  v->type = type;
  v->x = (stbtt_int16) x;
 ...
 int stbtt_IsGlyphEmpty(const stbtt_fontinfo * info, int glyph_index) 
{
 ...
 static int stbtt__close_shape(stbtt_vertex * vertices, int num_vertices,
                                 ...
                                 stbtt_int32 sy, stbtt_int32 scx,
                                 stbtt_int32 scy, stbtt_int32 cx,
                                 stbtt_int32 cy) 
{
  if (start_off) {
    if (was_off)
 ...
 int stbtt_GetGlyphShape(const stbtt_fontinfo * info, int glyph_index,
                           stbtt_vertex ** pvertices) 
{
   ...
   *pvertices = NULL;
   ...
   if (numberOfContours > 0) {
    ...
    stbtt_int32 ins, i, j = 0, m, n, next_move, was_off = 0, off, start_off =
      0;
    ...
    num_vertices =
      stbtt__close_shape(vertices, num_vertices, was_off, start_off, sx, sy,
                         scx, scy, cx, cy);
    ...
    while (more) {
      ...
      if (comp_num_verts > 0) {
          ...
          // Append vertices.
          tmp =
          (stbtt_vertex *) STBTT_malloc((num_vertices + comp_num_verts) *
                               ...
                               int *advanceWidth, int *leftSideBearing) 
{
 ...
 int stbtt_GetGlyphKernAdvance(const stbtt_fontinfo * info, int glyph1,
                                 int glyph2) 
{
 ...
 int stbtt_GetCodepointKernAdvance(const stbtt_fontinfo * info, int ch1,
                                     int ch2) 
{
...
}  void stbtt_GetFontVMetrics(const stbtt_fontinfo * info, int *ascent,
                                 int *descent, int *lineGap) 
{
  if (ascent)
 ...
 void stbtt_GetFontBoundingBox(const stbtt_fontinfo * info, int *x0, int *y0,
                                 int *x1, int *y1) 
{
...
}  float stbtt_ScaleForPixelHeight(const stbtt_fontinfo * info,
                                      float height) 
{
  int fheight =
    ttSHORT(info->data + info->hhea + 4) - ttSHORT(info->data + info->hhea + 6);
  return (float) height / fheight;
}  float stbtt_ScaleForMappingEmToPixels(const stbtt_fontinfo * info,
                                            float pixels) 
{
                              ...
                              int *ix1, int *iy1) 
{
  stbtt_GetGlyphBitmapBoxSubpixel(font, glyph, scale_x, scale_y, 0.0f, 0.0f,
                                   ix0, iy0, ix1, iy1);
}  void stbtt_GetCodepointBitmapBoxSubpixel(const stbtt_fontinfo * font,
                                               int codepoint, float scale_x,
                                               float scale_y, float shift_x,
                                               float shift_y, int *ix0,
                                               int *iy0, int *ix1, int *iy1) 
{
  stbtt_GetGlyphBitmapBoxSubpixel(font, stbtt_FindGlyphIndex(font, codepoint),
                                   ...
                                   ix1, iy1);
}  void stbtt_GetCodepointBitmapBox(const stbtt_fontinfo * font,
                                       int codepoint, float scale_x,
                                       float scale_y, int *ix0, int *iy0,
                                       int *ix1, int *iy1) 
{
          ...
          if (i == j) {
            ...
            if (i >= 0)        // add antialiasing for x0
              scanline[i] =
                scanline[i] +
                (stbtt_uint8) (((FIX -
                                 (x0 & FIXMASK)) * max_weight) >> FIXSHIFT);
 ...
 static int stbtt__edge_compare(const void *p, const void *q) 
{
 ...
 static void stbtt__rasterize(stbtt__bitmap * result, stbtt__point * pts,
                                ...
                                int off_x, int off_y, int invert,
                                void *userdata) 
{
 ...
 static void stbtt__add_point(stbtt__point * points, int n, float x,
                                float y) 
{
  if (!points)
                                     ...
                                     void *userdata) 
{
   ...
   *contour_lengths =
   ...
   return points;
error:STBTT_free(points, userdata);
                                       ...
                                       int *yoff) 
{
 ...
 void stbtt_MakeGlyphBitmapSubpixel(const stbtt_fontinfo * info,
                                      unsigned char *output, int out_w,
                                      int out_h, int out_stride, float scale_x,
                                      float scale_y, float shift_x,
                                      float shift_y, int glyph) 
{
 ...
 void stbtt_MakeGlyphBitmap(const stbtt_fontinfo * info,
                                                      ...
                                                      int *yoff) 
{
 ...
 void stbtt_MakeCodepointBitmapSubpixel(const stbtt_fontinfo * info,
                                          unsigned char *output, int out_w,
                                          int out_h, int out_stride,
                                          float scale_x, float scale_y,
                                          float shift_x, float shift_y,
                                 ...
                                 stbtt_bakedchar * chardata) 
{
 ...
 void stbtt_GetBakedQuad(stbtt_bakedchar * chardata, int pw, int ph,
...
static stbtt_int32 stbtt__CompareUTF8toUTF16_bigendian_prefix(const stbtt_uint8
                                                              * s1,
                                                              stbtt_int32 len1,
                                                              const stbtt_uint8
                                                              * s2,
  ...
  
{
 ...
 int stbtt_CompareUTF8toUTF16_bigendian(const char *s1, int len1,
                                          const char *s2, int len2) 
{
  return len1 ==
    stbtt__CompareUTF8toUTF16_bigendian_prefix((const stbtt_uint8 *) s1, len1,
                                    ...
                                    int languageID, int nameID) 
{
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (platformID == ttUSHORT(fc + loc + 0)
         && encodingID == ttUSHORT(fc + loc + 2)  &&languageID ==
         ttUSHORT(fc + loc + 4) && nameID == ttUSHORT(fc + loc + 6)) {
 ...
 static int stbtt__matchpair(stbtt_uint8 * fc, stbtt_uint32 nm,
                               stbtt_uint8 * name, stbtt_int32 nlen,
  ...
  stbtt_int32 stringOffset = nm + ttUSHORT(fc + nm + 4);
   for (i = 0; i < count; ++i) {
    ...
    if (id == target_id) {
        ...
        stbtt_int32 slen = ttUSHORT(fc + loc + 8), off =
          ttUSHORT(fc + loc + 10);
         
          // check if there's a prefix match
          stbtt_int32 matchlen =
          stbtt__CompareUTF8toUTF16_bigendian_prefix(name, nlen,
                                                     fc + stringOffset + off,
                                                     slen);
        if (matchlen >= 0) {
            ...
            stbtt_int32 slen = ttUSHORT(fc + loc + 12 + 8), off =
              ttUSHORT(fc + loc + 12 + 10);
            if (slen == 0) {
              if (matchlen == nlen)
                return 1;
